...
print(f"固定配置已加载 | 目标数量: {TOTAL_ACCOUNTS} | 邮箱域名: {MAIL_DOMAIN}")
...
def _generate_worker_email() -> str:
    ...
    chars = list(random.choices(string.ascii_lowercase, k=local_len))
    for _ in range(random.choice([1, 2])):
        pos = random.randint(2, len(chars) - 1)
        chars.insert(pos, random.choice(string.digits))
    return f"{''.join(chars)}@{MAIL_DOMAIN}"
...
def _poll_otp_from_worker(email: str) -> Optional[str]:
    ...
    for attempt in range(MAIL_POLL_MAX_ATTEMPTS):
        try:
            ...
            if resp.status_code == 200:
                data = resp.json()
                code = _extract_otp_code_from_payload(data if isinstance(data, dict) else {})
                if code:
                    logger.info("收到 Worker 验证码: %s", code)
                    return code
        except Exception:
            pass

        if attempt < MAIL_POLL_MAX_ATTEMPTS - 1:
            time.sleep(MAIL_POLL_SECONDS)

    logger.warning("等待 Worker 验证码超时 | email=%s", email)
    return None
...
def oauth_login(email: str, password: str, proxy: str = "") -> Optional[Tuple[str, str]]:
    ...
    if page_type == "email_otp_verification" or "email-verification" in continue_url:
        ...
        if rv.status_code != 200:
            logger.warning("[登录] OTP 验证失败: HTTP %s", rv.status_code)
            return None
        try:
            d2 = rv.json()
...
def build_accounts_jsonl_record(email: str, password: str, access_token: str, refresh_token: str = "") -> Dict[str, Any]:
    return {
...
def process_one(proxy: str = "") -> bool:
    ...
    if not reg.register(email=email, password=password):
