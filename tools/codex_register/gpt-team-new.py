...
def _resolve_register_email(email: str) -> str:
    return str(email or "").strip().lower()


def _extract_otp_code_from_payload(payload: Dict[str, Any]) -> str:
    if not isinstance(payload, dict):
        return ""
    keys = ("otp_code", "code", "otp", "verification_code")
    for key in keys:
        val = payload.get(key)
        if val:
...
def _poll_otp_from_worker(email: str) -> Optional[str]:
    resolved_email = _resolve_register_email(email)
    if not resolved_email:
        return None

    url = f"{MAIL_WORKER_BASE_URL}/v1/code"
    headers = {
        "Authorization": f"Bearer {MAIL_WORKER_TOKEN}",
        "Accept": "application/json",
    }

    for _ in range(MAIL_POLL_MAX_ATTEMPTS):
        try:
            ...
            if resp.status_code == 200:
                payload = resp.json()
                code = _extract_otp_code_from_payload(payload if isinstance(payload, dict) else {})
                if code:
                    logger.info("收到Worker验证码: %s | email=%s", code, resolved_email)
                    return code
        except Exception as e:
            logger.warning("轮询Worker OTP异常: %s | email=%s", e, resolved_email)
        time.sleep(MAIL_POLL_SECONDS)

    logger.warning("Worker OTP轮询超时 | email=%s", resolved_email)
    return None


def _generate_worker_email() -> str:
    local_len = random.randint(10, 14)
    local_chars = list(random.choices(string.ascii_lowercase + string.digits, k=local_len))
    for _ in range(random.choice([1, 2])):
        pos = random.randint(1, len(local_chars) - 1)
        local_chars.insert(pos, random.choice(string.digits))
...
class ProtocolRegistrar:
    ...
    def register(
        ...
        code = _poll_otp_from_worker(email)
        if not code:
...
def perform_http_oauth_login(
    ...
    if page_type == "email_otp_verification" or "email-verification" in continue_url:
        ...
        # 必须先触发发送 OTP 邮件，否则等待轮询里可能收不到新验证码
...
def chatgpt_http_login(
    ...
    if page_type == "email_otp_verification" or "email-verification" in continue_url:
        ...
        if rv.status_code != 200:
            logger.warning(f"  [{tag}] OTP验证失败: HTTP %s | email=%s", rv.status_code, email)
            return "", ""
        try:
            d2 = rv.json()
...
def refresh_team_session_http(team):
    ...
    - 有 password：走密码登录流程
    - 无 password：直接走邮箱 OTP 登录流程（OTP 统一从 Worker 轮询）
    """
